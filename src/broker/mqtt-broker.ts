...
export class MqttBroker implements Broker {
    ...
    private constructor(private options: MqttBrokerOptions, private logger: Logger) {
        ...
        this.client.on('connect', () => {
            this.logger.info('✅ Fully connected to AWS IoT MQTT!');
            setTimeout(() => {
                this.logger.info('📥 Attempting subscription...');
                this.client.subscribe("test/topic", { qos: 1 }, (err, granted) => {
                    if (err) {
        ...
        this.client.on('reconnect', () => {
            this.logger.info('🔄 Reconnecting to AWS IoT MQTT...');
        });
    ...
    static getInstance(options: MqttBrokerOptions, logger: Logger): MqttBroker {
        if (!MqttBroker.instance) {
            MqttBroker.instance = new MqttBroker(options, logger);
        }
